...
CLASS_NAME = {"breech_face": 0, "firing_pin_impression": 1, "firing_pin_drag": 2}
...
class CustomDataset(utils.Dataset):
    def load_custom(self, dataset_dir, subset):
        ...
        for class_name in CLASS_NAME.keys():
            self.add_class("data", CLASS_NAME[class_name], class_name)
        ...
        annotations_dict = json.load(
            open(os.path.join(dataset_dir, "annotations.json"))
        )
        ...
        for a in annotations:
            ...
            class_names_each_region = [
                r["region_attributes"]["type"] for r in a["regions"].values()
            ]
            print(class_names_each_region)
            # class_names_each_region = [r['region_attributes']['type'] if 'type' in r['region_attributes'].keys()
            ...
            self.add_image(
                ...
                height=height,
                polygons=polygons,
                class_names=class_names_each_region,
            )
            # break
...
def detect_and_draw_image(model, image_path=None):
    ...
    if image_path:
        ...
        image_result = display_instances(
            image,
            r["rois"],
            r["masks"],
            r["class_ids"],
...
if __name__ == "__main__":
    ...
    if args.command == "train":
        ...
        print(
            "'{}' is not recognized. "
            "Use 'train', 'splash', 'splash_movie' or 'classic_movie'".format(
                args.command
            )
        )
